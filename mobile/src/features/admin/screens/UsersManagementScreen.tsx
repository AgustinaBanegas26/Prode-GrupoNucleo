...
export function UsersManagementScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    return users.filter((u) => {
      ...
      if (!q) return true;
      return `${u.numeroEmpleado} ${u.nombre} ${u.apellido}`.toLowerCase().includes(q);
    });
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await upsert({ ...form, numeroEmpleado: form.numeroEmpleado.trim(), nombre: form.nombre.trim(), apellido: form.apellido.trim() });
      log({ action: existed ? 'update' : 'create', module: 'users', title: existed ? 'Usuario actualizado' : 'Usuario creado', detail: `${form.nombre.trim()} ${form.apellido.trim()}` });
  ...
  const confirmDelete = (u: AppUser) =>
    Alert.alert('Eliminar usuario', `${u.nombre} ${u.apellido}`, [
      ...
      { text: 'Eliminar', style: 'destructive', onPress: async () => {
        await remove(u.id);
        log({ action: 'delete', module: 'users', title: 'Usuario eliminado', detail: `${u.nombre} ${u.apellido}` });
      }},
    ]);
  ...
  const handleBlockToggle = async (u: AppUser) => {
    ...
    log({ action: 'toggle', module: 'users', title: next ? 'Usuario desbloqueado' : 'Usuario bloqueado', detail: `${u.nombre} ${u.apellido}` });
  };

  const handleResetPassword = async (u: AppUser) => {
    const next = await resetPassword(u.id);
    log({ action: 'update', module: 'users', title: 'Contraseña restablecida', detail: `${u.nombre} ${u.apellido}` });
  ...
  const renderUserItem = ({ item, index }: { item: AppUser; index: number }) => {
    const isActive = item.activo;
    return (
      <FadeSlide delay={index * 35}>
        <View style={[u.card, { backgroundColor: cardBg, borderColor: cardBorder, shadowColor: CELESTE }]}>
          <View style={u.topRow}>
            {/* Avatar con iniciales */}
            <View style={[u.initials, { backgroundColor: isDark ? 'rgba(110,198,255,0.12)' : '#EBF5FF' }]}>
              <Text style={[u.initialsText, { color: CELESTE_DARK }]}>
                {item.nombre.slice(0, 1)}{item.apellido.slice(0, 1)}
              </Text>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={[u.name, { color: textPrimary }]}>{item.nombre} {item.apellido}</Text>
              <Text style={[u.meta, { color: textMuted }]}>Nº {item.numeroEmpleado}  ·  {item.rol === 'admin' ? '👑 Admin' : 'Usuario'}</Text>
            </View>
            {/* Badge estado */}
            <View style={[u.badge, { backgroundColor: isActive ? (isDark ? 'rgba(110,198,255,0.12)' : '#EBF5FF') : 'rgba(239,68,68,0.12)' }]}>
              <View style={[u.dot, { backgroundColor: isActive ? CELESTE_DARK : '#ef4444' }]} />
              <Text style={[u.badgeText, { color: isActive ? CELESTE_DARK : '#ef4444' }]}>
          ...
          {/* Separador */}
          <View style={[u.divider, { backgroundColor: isDark ? 'rgba(110,198,255,0.08)' : 'rgba(110,198,255,0.15)' }]} />

          {/* Acciones */}
          <View style={u.actions}>
            <ActionBtn icon="pencil"  label="Editar"  color={CELESTE_DARK} onPress={() => openEdit(item)} />
            <ActionBtn icon="key"     label="Reset"   color="#f59e0b" onPress={() => handleResetPassword(item)} />
            <ActionBtn icon={isActive ? 'lock' : 'lock-open-variant'} label={isActive ? 'Bloq.' : 'Desbloq.'} color={isActive ? '#f59e0b' : CELESTE_DARK} onPress={() => handleBlockToggle(item)} />
            <ActionBtn icon="delete"  label="Eliminar" color={RED}    onPress={() => confirmDelete(item)} />
          </View>
        </View>
      </FadeSlide>
    );
  ...
  return (
    <View style={[s.root, { backgroundColor: bg }]}>
      ...
      {/* ── Header idéntico a la app usuario ──────────── */}
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        <View style={s.section}>
          ...
          {/* Búsqueda */}
          <View style={[s.searchBox, { backgroundColor: searchBg, borderColor: isDark ? 'rgba(110,198,255,0.15)' : 'rgba(110,198,255,0.3)' }]}>
          ...
          {/* Filtros — igual que los tabs de posiciones.tsx */}
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={s.empty}>
                ...
                <Text style={[s.emptyText, { color: textMuted }]}>No hay usuarios para mostrar</Text>
              </View>
            }
          />
        </View>
      ...
      <Modal visible={modalVisible} transparent animationType="slide" onRequestClose={() => setModalVisible(false)}>
        <View style={m.backdrop}>
          <View style={[m.sheet, {
            ...
            {/* Handle */}
            <View style={[m.handle, { backgroundColor: isDark ? 'rgba(255,255,255,0.15)' : 'rgba(0,0,0,0.12)' }]} />

            <View style={m.sheetHeader}>
              <Text style={[m.sheetTitle, { color: textPrimary }]}>
                {users.some((u) => u.id === form.id) ? '✏️  Editar usuario' : '➕  Crear usuario'}
              </Text>
              <Pressable onPress={() => setModalVisible(false)} style={[m.closeBtn, { backgroundColor: isDark ? 'rgba(255,255,255,0.08)' : '#F5F7FA' }]}>
            ...
            <View style={m.body}>
              ...
              <View style={m.pillRow}>
                {([true, false] as const).map((st) => (
                  <Pressable key={String(st)} onPress={() => setForm((p) => ({ ...p, activo: st }))}
                    style={[m.pill, form.activo === st && { backgroundColor: isDark ? 'rgba(61,165,245,0.18)' : '#EBF5FF', borderColor: CELESTE_DARK }]}>
                    <Text style={[m.pillTxt, { color: form.activo === st ? CELESTE_DARK : textMuted }]}>{st ? 'Activo' : 'Bloqueado'}</Text>
                  </Pressable>
                ))}
              </View>
              <View style={m.pillRow}>
                {(['usuario', 'admin'] as const).map((r) => (
                  <Pressable key={r} onPress={() => setForm((p) => ({ ...p, rol: r }))}
                    style={[m.pill, form.rol === r && { backgroundColor: isDark ? 'rgba(61,165,245,0.18)' : '#EBF5FF', borderColor: CELESTE_DARK }]}>
                    <Text style={[m.pillTxt, { color: form.rol === r ? CELESTE_DARK : textMuted }]}>{r === 'usuario' ? 'Usuario' : '👑 Admin'}</Text>
                  </Pressable>
                ))}
              </View>
            </View>
            ...
            <View style={m.footer}>
              <Pressable onPress={handleSave} disabled={saving} style={m.saveBtn}>
                <LinearGradient colors={[CELESTE_DARK, DEEP_BLUE]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={m.saveBtnGrad}>
                  <Text style={m.saveBtnText}>{saving ? 'Guardando...' : 'Guardar'}</Text>
                </LinearGradient>
              </Pressable>
            </View>
          </View>
...
function ActionBtn({ icon, label, color, onPress }: { icon: string; label: string; color: string; onPress: () => void }) {
  return (
    <Pressable onPress={onPress} style={[ab.btn, { backgroundColor: `${color}15`, borderColor: `${color}30` }]}>
...
const ab = StyleSheet.create({
  btn: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingVertical: 7, paddingHorizontal: 9, borderRadius: 10, borderWidth: 1, flex: 1 },
  txt: { fontSize: 11, fontWeight: '700' },
});

const u = StyleSheet.create({
  card:         { borderRadius: 20, borderWidth: 1, padding: 16, gap: 12, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.10, shadowRadius: 8, elevation: 3 },
...
const s = StyleSheet.create({
  ...
  addGrad:   { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 12, paddingVertical: 9 },
  addText:   { color: '#fff', fontSize: 12, fontWeight: '700' },
  section:   { padding: 16, gap: 12 },
  searchBox: { flexDirection: 'row', alignItems: 'center', gap: 10, borderWidth: 1, borderRadius: 14, paddingHorizontal: 14, shadowColor: CELESTE, shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 4, elevation: 2 },
  searchInput: { flex: 1, height: 46, fontSize: 14 },
  tabBar:    { flexDirection: 'row', borderRadius: 20, padding: 4, alignSelf: 'stretch' },
  tabItem:   { flex: 1, paddingVertical: 9, borderRadius: 18, alignItems: 'center' },
...
const m = StyleSheet.create({
  ...
  sheet:       { borderTopLeftRadius: 28, borderTopRightRadius: 28, paddingBottom: 36, borderWidth: 1 },
  ...
  pill:        { flex: 1, paddingVertical: 10, borderRadius: 12, borderWidth: 1.5, borderColor: 'rgba(0,0,0,0.10)', alignItems: 'center' },
