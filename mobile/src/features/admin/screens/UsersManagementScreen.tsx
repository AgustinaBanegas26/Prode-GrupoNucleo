        ...
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert("Error", e instanceof Error ? e.message : "No se pudo resetear la contraseña");
            }
          },
        },
      ],
        ...
        <FadeSlide delay={0}>
          <View style={s.headerRow}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={s.sub}>{users.length} clientes registrados</Text>
            </View>
...
function UserModal({
  ...
  const handleSave = async () => {
    if (!form.nombre.trim() || !form.clienteId.trim()) {
      Alert.alert("Error", "Completá nombre e ID de cliente");
      return;
    }
    try {
      await onSave(form);
    } catch {
      // onSave ya muestra el error
    }
  };
      ...
      <Pressable style={m.backdrop} onPress={onClose}>
        ...
        >
          ...
          <ScrollView style={m.body}>
            ...
            {!user ? (
              <Text style={[m.hint, { color: theme.colors.muted }]}>
                Contraseña inicial: clientesgn123 (primer login)
              </Text>
            ) : null}
...
const m = StyleSheet.create({
  ...
  hint: { fontSize: 12, fontWeight: "500", marginBottom: 10, lineHeight: 18 },
  input: {
    borderWidth: 1,
    borderRadius: 14,
