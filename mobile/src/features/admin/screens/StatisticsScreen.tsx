...
import { useStatistics, useStatisticsRealtime } from '../../content/api/statistics';
...
export function StatisticsScreen() {
  ...
  const statsData = [
    { icon: 'account-multiple', value: stats?.totalUsers || 0, label: 'Usuarios Totales' },
    { icon: 'account-check',    value: stats?.activeUsers || 0, label: 'Usuarios Activos' },
    { icon: 'soccer',           value: stats?.totalPredictions || 0, label: 'Pronósticos' },
      ...
      <View style={[s.content, { backgroundColor: bg }]}>
        ...
        <View style={s.statsRow}>
          {statsData.map((stat, i) => (
            <View key={stat.label} style={[s.statCard, { backgroundColor: cardBg, borderColor: cardBorder, shadowColor: CELESTE }]}>
              <View style={[s.statIcon, { backgroundColor: isDark ? 'rgba(110,198,255,0.12)' : '#EBF5FF' }]}>
                <MaterialCommunityIcons name={stat.icon as any} size={20} color={CELESTE_DARK} />
              </View>
              <Text style={[s.statValue, { color: CELESTE_DARK }]}>{stat.value}</Text>
              <Text style={[s.statLabel, { color: theme.colors.muted }]}>{stat.label}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={[s.card, { backgroundColor: cardBg, borderColor: cardBorder, shadowColor: CELESTE }]}>
          ...
          <View style={s.precRow}>
            <Text style={[s.precValue, { color: CELESTE_DARK }]}>{stats?.avgAccuracy || 0}%</Text>
          ...
          <View style={[s.track, { backgroundColor: isDark ? 'rgba(110,198,255,0.12)' : '#EBF5FF' }]}>
            <LinearGradient colors={[CELESTE, CELESTE_DARK]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={[s.fill, { width: `${stats?.avgAccuracy || 0}%` }]} />
          </View>
          <View style={s.legend}>
